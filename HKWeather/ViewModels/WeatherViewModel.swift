...
final class WeatherViewModel: ObservableObject {
    ...
    private func getWeather(coord: CLLocationCoordinate2D?) {
        var urlString = ""
        if let coord = coord {
            urlString = WeatherApi.getForecastWeatherURL(lat: coord.latitude, lon: coord.longitude)
        } else {
            urlString = WeatherApi.getForecastWeatherURL(lat: 0, lon: 0)
        }
    ...
    // MARK: fields
    var date: String {
        Utils.defaultDateFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(weather.current.dt)) )
    }
    ...
    func getTimeFor(timestamp: Int) -> String {
        Utils.timeFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(timestamp)))
    }

    func getDayFor(timestamp: Int) -> String {
        Utils.dayFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(timestamp)))
    }
    
    func getDayNumber(timestamp: Int) -> String {
        Utils.dayNumberFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(timestamp)))
    }
